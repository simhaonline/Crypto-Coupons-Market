...
#if defined(NDEBUG)
#error "CRYPTOCOUPON cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uCCM) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight ==0)
        ...
        nSubsidy = 80 * COIN;
    else if (nHeight > 259221 && nHeight <= 388800)
        nSubsidy = 40 * COIN;
    else if (nHeight > 388801 && nHeight <= 518400)
        nSubsidy = 20 * COIN;
    else if (nHeight > 518401 && nHeight <= 648000)
        nSubsidy = 10 * COIN;
    else if (nHeight > 648001 && nHeight <=  907200)
        nSubsidy = 5 * COIN;
    else if (nHeight > 907201)
        nSubsidy = 2.5 * COIN;
    else
        nSubsidy = 2.5 * COIN;
...
{
    RenameThread("CRYPTOCOUPON-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // CCM: We use certain sporks during IBD, so check to see if they are
